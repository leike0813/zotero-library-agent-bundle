...
```console
zotero-bridge synthesis topic get-review-input [--endpoint <ENDPOINT>] [--operation-id <ID>] [--profile <PATH>] [--schema] [--query <JSON_OR_FILE>]
```
...
```console
zotero-bridge synthesis topic get-review-input --query '{}'
```
...
```json
{
  ...
  "binding": "passthrough",
  "category": "read",
  "command": "synthesis topic get-review-input",
  "composition": null,
  "danger": "none",
  "effects": [
    {
  ...
  "summary": "Read review workflow input from Synthesis",
  "targets": [
    {
      "kind": "capability",
      "target": "topics.get_review_input"
    }
  ]
}
```

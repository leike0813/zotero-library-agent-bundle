...
```console
zotero-bridge synthesis topic get-report [--endpoint <ENDPOINT>] [--operation-id <ID>] [--profile <PATH>] [--schema] [--query <JSON_OR_FILE>]
```
...
```json
{
  ...
  "properties": {
    ...
    "capability": {
      "const": "topics.get_report"
    },
    "data": {
      "additionalProperties": true,
      "description": "Result data owned by topics.get_report.",
      "properties": {
        "hasMore": {
          "type": "boolean"
        },
...
```console
zotero-bridge synthesis topic get-report --query '{}'
```
...
```json
{
  ...
  "binding": "passthrough",
  "category": "read",
  "command": "synthesis topic get-report",
  "composition": null,
  "danger": "none",
  "effects": [
    {
  ...
  "summary": "Read one topic synthesis report markdown body",
  "targets": [
    {
      "kind": "capability",
      "target": "topics.get_report"
    }
  ]
}
```
...
- Canonical argv path: `synthesis` `topic` `get-report`.
...
- Operational aliases: `synthesis topic get-report`, `synthesis`, `topic`, `get-report`, `query`, `JSON_OR_FILE`.

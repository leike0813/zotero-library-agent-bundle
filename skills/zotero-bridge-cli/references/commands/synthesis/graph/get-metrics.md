...
```console
zotero-bridge synthesis graph get-metrics [--endpoint <ENDPOINT>] [--operation-id <ID>] [--profile <PATH>] [--schema] [--query <JSON_OR_FILE>]
```
...
```json
{
  ...
  "properties": {
    ...
    "capability": {
      "const": "citation_graph.get_metrics"
    },
    "data": {
      "additionalProperties": true,
      "description": "Result data owned by citation_graph.get_metrics.",
      "properties": {
        "hasMore": {
          "type": "boolean"
        },
...
```console
zotero-bridge synthesis graph get-metrics --query '{}'
```
...
```json
{
  ...
  "binding": "passthrough",
  "category": "read",
  "command": "synthesis graph get-metrics",
  "composition": null,
  "danger": "none",
  "effects": [
    {
  ...
  "summary": "Read citation graph metrics for selected papers",
  "targets": [
    {
      "kind": "capability",
      "target": "citation_graph.get_metrics"
    }
  ]
}
```
...
- Canonical argv path: `synthesis` `graph` `get-metrics`.
- Output boundary: `cursor`; governed details: {"continuation":["data.nextCursor","data.hasMore","data.returned","data.total","data.limit"],"cursorInput":"cursor","defaultLimit":25,"maxLimit":100,"section":"data.metrics","strategy":"cursor"}.
- Pagination: `cursor`.
- Category: `read`; danger: `none`.
- Structured binding mode: `passthrough`.
- Intent visibility: `visible`.
- Operational aliases: `synthesis graph get-metrics`, `synthesis`, `graph`, `get-metrics`, `query`, `JSON_OR_FILE`.

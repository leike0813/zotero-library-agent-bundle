...
```console
zotero-bridge synthesis graph get-slice [--endpoint <ENDPOINT>] [--operation-id <ID>] [--profile <PATH>] [--schema] [--query <JSON_OR_FILE>]
```
...
```json
{
  ...
  "properties": {
    ...
    "capability": {
      "const": "citation_graph.get_slice"
    },
    "data": {
      "additionalProperties": true,
...
```console
zotero-bridge synthesis graph get-slice --query '{}'
```
...
```json
{
  ...
  "binding": "passthrough",
  "category": "read",
  "command": "synthesis graph get-slice",
  "composition": null,
  "danger": "none",
  "effects": [
    {
  ...
  "summary": "Read a Synthesis citation graph slice",
  "targets": [
    {
      "kind": "capability",
      "target": "citation_graph.get_slice"
    }
  ]
}
```
...
- Canonical argv path: `synthesis` `graph` `get-slice`.
...
- Operational aliases: `synthesis graph get-slice`, `synthesis`, `graph`, `get-slice`, `query`, `JSON_OR_FILE`.

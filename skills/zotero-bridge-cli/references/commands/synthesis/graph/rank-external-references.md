...
```console
zotero-bridge synthesis graph rank-external-references [--endpoint <ENDPOINT>] [--operation-id <ID>] [--profile <PATH>] [--schema] [--query <JSON_OR_FILE>]
```
...
```json
{
  ...
  "properties": {
    ...
    "capability": {
      "const": "citation_graph.rank_external_references"
    },
    "data": {
      "additionalProperties": true,
      "description": "Result data owned by citation_graph.rank_external_references.",
      "properties": {
        "hasMore": {
          "type": "boolean"
        },
...
```console
zotero-bridge synthesis graph rank-external-references --query '{}'
```
...
```json
{
  ...
  "binding": "passthrough",
  "category": "read",
  "command": "synthesis graph rank-external-references",
  "composition": null,
  "danger": "none",
  "effects": [
    {
  ...
  "summary": "Rank external references from the citation graph",
  "targets": [
    {
      "kind": "capability",
      "target": "citation_graph.rank_external_references"
    }
  ]
}
```
...
- Canonical argv path: `synthesis` `graph` `rank-external-references`.
- Output boundary: `cursor`; governed details: {"continuation":["data.nextCursor","data.hasMore","data.returned","data.total","data.limit"],"cursorInput":"cursor","defaultLimit":25,"maxLimit":100,"section":"data.references","strategy":"cursor"}.
- Pagination: `cursor`.
- Category: `read`; danger: `none`.
- Structured binding mode: `passthrough`.
- Intent visibility: `visible`.
- Operational aliases: `synthesis graph rank-external-references`, `synthesis`, `graph`, `rank-external-references`, `query`, `JSON_OR_FILE`.

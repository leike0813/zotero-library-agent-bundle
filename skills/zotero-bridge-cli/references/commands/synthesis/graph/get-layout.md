...
```console
zotero-bridge synthesis graph get-layout [--endpoint <ENDPOINT>] [--operation-id <ID>] [--profile <PATH>] [--schema] [--query <JSON_OR_FILE>]
```
...
```json
{
  ...
  "properties": {
    ...
    "capability": {
      "const": "citation_graph.get_layout"
    },
    "data": {
      "additionalProperties": true,
      "description": "Result data owned by citation_graph.get_layout.",
      "properties": {
        "diagnostics": {
          "additionalProperties": true,
          "properties": {
...
```console
zotero-bridge synthesis graph get-layout --query '{}'
```
...
```json
{
  ...
  "binding": "passthrough",
  "category": "read",
  "command": "synthesis graph get-layout",
  "composition": null,
  "danger": "none",
  "effects": [
    {
  ...
  "summary": "Read persisted citation graph layout coordinates",
  "targets": [
    {
      "kind": "capability",
      "target": "citation_graph.get_layout"
    }
  ]
}
```
...
- Canonical argv path: `synthesis` `graph` `get-layout`.
...
- Operational aliases: `synthesis graph get-layout`, `synthesis`, `graph`, `get-layout`, `query`, `JSON_OR_FILE`.

...
```console
zotero-bridge synthesis resolver resolve [--endpoint <ENDPOINT>] [--operation-id <ID>] [--profile <PATH>] [--schema] [--query <JSON_OR_FILE>]
```
...
```json
{
  ...
  "properties": {
    ...
    "data": {
      "additionalProperties": true,
      "description": "Result data owned by resolvers.resolve.",
      "properties": {
        "candidates": {
          "type": "array"
        },
...
```console
zotero-bridge synthesis resolver resolve --query '{}'
```
...
```json
{
  ...
  "summary": "Resolve a topic resolver into a paper set",
  "targets": [
    {
      "kind": "capability",
      "target": "resolvers.resolve"
    }
  ]
}
```
...
- Canonical argv path: `synthesis` `resolver` `resolve`.
- Output boundary: `cursor`; governed details: {"continuation":["data.nextCursor","data.hasMore","data.returned","data.total","data.limit"],"cursorInput":"cursor","defaultLimit":25,"maxLimit":100,"section":"data.candidates","strategy":"cursor"}.
- Pagination: `cursor`.
- Category: `read`; danger: `none`.
- Structured binding mode: `passthrough`.
- Intent visibility: `visible`.
- Operational aliases: `synthesis resolver resolve`, `synthesis`, `resolver`, `resolve`, `query`, `JSON_OR_FILE`.

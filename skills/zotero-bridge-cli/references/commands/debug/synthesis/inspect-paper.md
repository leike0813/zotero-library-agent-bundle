...
```console
zotero-bridge debug synthesis inspect-paper [--endpoint <ENDPOINT>] [--operation-id <ID>] [--profile <PATH>] [--schema] [--input <JSON_OR_FILE>]
```
...
```json
{
  ...
  "properties": {
    ...
    "capability": {
      "const": "debug.synthesis.paper.inspect"
    },
    "data": {
      "additionalProperties": true,
...
```console
zotero-bridge debug synthesis inspect-paper --input '{}'
```
...
```json
{
  ...
  "binding": "passthrough",
  "category": "debug",
  "command": "debug synthesis inspect-paper",
  "composition": null,
  "danger": "none",
  "effects": [
    {
  ...
  "summary": "Inspect one debug Synthesis paper",
  "targets": [
    {
      "kind": "capability",
      "target": "debug.synthesis.paper.inspect"
    }
  ]
}
```
...
- Output boundary: `limit`; governed details: {"defaultLimit":25,"fileField":"data.delivery.bundle","maxLimit":100,"section":"data.repository.artifactSidecars","strategy":"limit","truncatedField":"data.truncated"}.
- Pagination: `none`.
- Category: `debug`; danger: `none`.
- Structured binding mode: `passthrough`.
- Intent visibility: `hidden`.
- Operational aliases: `debug synthesis inspect-paper`, `debug`, `synthesis`, `inspect-paper`, `input`, `JSON_OR_FILE`.

...
```console
zotero-bridge debug synthesis diff [--endpoint <ENDPOINT>] [--operation-id <ID>] [--profile <PATH>] [--schema] [--input <JSON_OR_FILE>]
```
...
```console
zotero-bridge debug synthesis diff --input '{}'
```
...
```json
{
  ...
  "summary": "Read debug-only Synthesis DB/cache differences",
  "targets": [
    {
      "kind": "capability",
      "target": "debug.synthesis.diff"
    }
  ]
}
```
...
- Canonical argv path: `debug` `synthesis` `diff`.
- Output boundary: `limit`; governed details: {"defaultLimit":25,"maxLimit":100,"section":"data.issues","strategy":"limit","truncatedField":"data.truncated"}.
- Pagination: `none`.
- Category: `debug`; danger: `none`.
- Structured binding mode: `passthrough`.
- Intent visibility: `hidden`.
- Operational aliases: `debug synthesis diff`, `debug`, `synthesis`, `diff`, `input`, `JSON_OR_FILE`.

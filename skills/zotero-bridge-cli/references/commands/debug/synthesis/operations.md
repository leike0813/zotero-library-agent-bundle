...
```json
{
  ...
  "properties": {
    ...
    "capability": {
      "const": "debug.synthesis.operations.list"
    },
    "data": {
      "additionalProperties": true,
...
- Canonical argv path: `debug` `synthesis` `operations`.
- Output boundary: `limit`; governed details: {"defaultLimit":25,"maxLimit":100,"section":"data.rows","strategy":"limit","truncatedField":"data.truncated"}.
- Pagination: `none`.
- Category: `debug`; danger: `none`.
- Structured binding mode: `passthrough`.
- Intent visibility: `hidden`.
- Operational aliases: `debug synthesis operations`, `debug`, `synthesis`, `operations`, `input`, `JSON_OR_FILE`.

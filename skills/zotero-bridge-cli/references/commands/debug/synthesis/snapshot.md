...
```console
zotero-bridge debug synthesis snapshot [--endpoint <ENDPOINT>] [--operation-id <ID>] [--profile <PATH>] [--schema] [--input <JSON_OR_FILE>]
```
...
```json
{
  ...
  "properties": {
    ...
    "capability": {
      "const": "debug.synthesis.snapshot"
    },
    "data": {
      "additionalProperties": true,
      "description": "Result data owned by debug.synthesis.snapshot.",
      "properties": {
        "cacheBasis": {
          "type": "array"
        },
...
```console
zotero-bridge debug synthesis snapshot --input '{}'
```
...
```json
{
  ...
  "binding": "passthrough",
  "category": "debug",
  "command": "debug synthesis snapshot",
  "composition": null,
  "danger": "none",
  "effects": [
    {
  ...
  "summary": "Read a debug-only Synthesis snapshot",
  "targets": [
    {
      "kind": "capability",
      "target": "debug.synthesis.snapshot"
    }
  ]
}
```
...
- Canonical argv path: `debug` `synthesis` `snapshot`.
- Output boundary: `limit`; governed details: {"defaultLimit":25,"fileField":"data.delivery.bundle","maxLimit":100,"section":"data.cacheBasis","strategy":"limit","truncatedField":"data.truncated"}.
- Pagination: `none`.
- Category: `debug`; danger: `none`.
- Structured binding mode: `passthrough`.
- Intent visibility: `hidden`.
- Operational aliases: `debug synthesis snapshot`, `debug`, `synthesis`, `snapshot`, `input`, `JSON_OR_FILE`.

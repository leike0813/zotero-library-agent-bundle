...
```console
zotero-bridge library items list [--endpoint <ENDPOINT>] [--operation-id <ID>] [--profile <PATH>] [--schema] [--query <JSON_OR_FILE>]
```
...
```json
{
  ...
  "properties": {
    ...
    "data": {
      "type": "object",
      "description": "Result data owned by library.list_items.",
...
```console
zotero-bridge library items list --query '{}'
```
...
```json
{
  "command": "library items list",
  "argv": [
    "library",
    "items",
    "list"
  ],
  "summary": "List compact Zotero library item summaries",
  "category": "read",
  "danger": "none",
  "invocationSchema": {
    "type": "object",
...
- Canonical argv path: `library` `items` `list`.
...
- Operational aliases: `library items list`, `library`, `items`, `list`, `query`, `JSON_OR_FILE`.
